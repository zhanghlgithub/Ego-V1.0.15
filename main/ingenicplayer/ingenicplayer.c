...
static char *put_music_list(char *data)
{
	...
	int music_state = -1;
	struct music_info *current_play_info = NULL;
	//struct music_info *play_info = NULL;
	int music_pos = 0;	//音乐的播放位置
	music_state = mozart_musicplayer_get_status(mozart_musicplayer_handler);	//新添加于2018.8.25号
	char *music_url = NULL;
	char *music_name = NULL;
	int play_index = -1;
	if(music_state == PLAYER_PLAYING)
	{
		current_play_info = mozart_musicplayer_musiclist_get_current_info(mozart_musicplayer_handler);
		if(current_play_info->music_url)
		{
			music_url = (char *)malloc(strlen(current_play_info->music_url)+1);
			memset(music_url,0,strlen(current_play_info->music_url)+1);
			strcpy(music_url,current_play_info->music_url);

			music_name = (char *)malloc(strlen(current_play_info->music_name)+1);
			memset(music_name,0,strlen(current_play_info->music_name)+1);
			strcpy(music_name,current_play_info->music_name);
		}
		
		music_pos = mozart_musicplayer_get_pos(mozart_musicplayer_handler);
		printf("\n正在播放音乐%s               播放的位置:%d      音乐的路径：%s\n",current_play_info->music_url,music_pos,music_url);
	}
	...
	if(songs != NULL){
		...
		if(0 != mozart_musicplayer_musiclist_get_length(mozart_musicplayer_handler))
		{
			if (mozart_musicplayer_get_status(mozart_musicplayer_handler) != PLAYER_STOPPED)
			{
				mozart_musicplayer_stop_playback(mozart_musicplayer_handler);  
		...
		printf("...................len:%d...............................",len);
	  	for (i = 0; i < len; i++) {
			...
			if(music_url){
				if(!strcmp(music_url,(char *)songurl)){
					play_index = i;
				}
			}
	...
	if(inlist == 1)
	{
		...
		{
			if(music_url != NULL){
				printf("\n播放的名字:%s \n音乐的路径：%s\n第几个位置：%d\n",music_name,music_url,play_index);
				
				//play_info = mozart_musiclist_get_info(0, (char *)music_name, (char *)music_url,NULL,NULL, NULL, NULL);
				//mozart_musicplayer_play_music_info(mozart_musicplayer_handler, play_info);
				mozart_musicplayer_play_index(mozart_musicplayer_handler,play_index);//播放音乐列表位于第index处的文件
			//	mozart_player_playurl(mozart_musicplayer_handler->player_handler, music_url); //播放url路径下的音乐？？
				//usleep(100*1000);
				mozart_musicplayer_set_seek(mozart_musicplayer_handler, music_pos); 
			}
		}
	}
...
static char *get_dev_config(void)
{
	...
	get_device_name(base_object);	//设备名称
 	json_object_object_add(base_object, "model", json_object_new_string("EGO-B1"));//设备型号
  	...
  	json_object_object_add(base_object, "charge", json_object_new_int(charge_state));		//电池状态
	...
	json_object_object_add(tower_object, "charge", json_object_new_int(g_tower_if_power));		//tower是否在充电
	//json_object_object_add(tower_object, "battery", json_object_new_int((int)info->Tower_power));		//电池电量
	json_object_object_add(tower_object, "battery", json_object_new_int(g_tower_power));		//电池电量
...
static char *edit_med_cfg(char *data)
{
	...
	name = get_object_string("name",NULL,data);
	model = get_object_int( "model", data);
	...
	if(model != -1)			//新添加于2018.8.25号
	{
		json_object_object_add(reply_object, "model", json_object_new_int(model));
	}
...
static int check_update(char *data)
{
	...
	if(type == 1 || type == 0)
	{
		if ((capacity > 60) || (g_power_on == 48))	//新添加于2018.7.30号
		{
...
static int reset_config(char *data)
{
	...
	if(type== 1||type == 3)   
	{
		...
		//删除历史播放列表
		system("rm -r /usr/data/free.txt");
		system("rm -r /usr/data/med.txt");
		system("rm -r /usr/data/sleep.txt");
		system("rm -r /usr/data/free_len.txt");
		system("rm -r /usr/data/med_len.txt");
		system("rm -r /usr/data/sleep_len.txt");
		//删除闹钟
		//system("rm -r /usr/data/alarm.list");
		//system("mkdir -p /usr/data/alarm.list");
...
#endif
	strncpy(new_name,local,len_l);	//在不建子文件夹的情况下可以这样使用，新添加于2018.8.1号
	printf("%s %s %d new_name:%s。。。。。。。。。。。。。。。。。。\n",__FILE__, __func__, __LINE__,new_name);	
...
static char *delect_music(char *data)
{
	...
	if(url != NULL)
	{
		...
		if(ret == 0){
			...
			/****删除失败***/
			json_object_object_add(reply_object, "res", json_object_new_int(0));
			json_object_object_add(reply_object, "reason", json_object_new_string("delete fail"));
		}
	}
	else{
		/*未找到此音乐文件*/
		json_object_object_add(reply_object, "dir", json_object_new_int(0));
		json_object_object_add(reply_object, "reason", json_object_new_string("file no exsit"));	
	}
...
static void set_music_position(char *data)
{
	int pos = 0;
	json_object *object = NULL;
	json_object *tmp = NULL;
	//char *reply_json = NULL;
	object = json_tokener_parse(data);
	if(json_object_object_get_ex(object,"pos",&tmp))
	{
		pos = json_object_get_int(tmp);
	}
	else
	{
		return;
	}
	mozart_musicplayer_set_seek(mozart_musicplayer_handler, pos);
	return;
}
  	...
  	switch(index){
	    ...
	    // 	break;
	  	case PK_AB_PUT_MUSIC_LIST:   //发送播放列表到base（0x56）
			...
			break;	 
		case PK_AB_SET_MUSIC_POS:
			set_music_position(data);	//设置音乐的播放进度
			break;
...
int mozart_ingenicplayer_notify_alarm(struct module_alarm_info *alarm)
{
	...
	/*****************************新添加于20188.10号**************************************************/
	json_object_object_add(object, "type", json_object_new_int(alarm_end));
	/******************************************************************************/
	json_object_object_add(reply_object, "data",object);

	mozart_appserver_notify("usercommand", (char *)json_object_get_string(reply_object));
	printf("回复给app的0x34协议：%s\n",(char *)json_object_get_string(reply_object));
//	json_object_put(object);
	json_object_put(reply_object);
...
static char *mozart_ingenicplayer_brain_data()
{
	...
	json_object_object_add(object, "a", json_object_new_int((int)(x_info->X_brain_concentration)));
    json_object_object_add(object, "m", json_object_new_int((int)(x_info->X_brain_attention)));
...
int mozart_ingenicplayer_wifi_state(unsigned char *data)
{
	...
	printf("tower传来的IP:%d.%d.%d.%d,%d",data[1],data[2],data[3],data[4],data[5]*256+data[6]);
