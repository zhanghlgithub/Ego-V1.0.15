...
//	struct music_info *info = NULL;
	...
	printf("发送来的闹钟歌曲的名字:%s\n",alarm->mname);
    url = (char *)malloc(strlen("/mnt/sdcard/alarm/") + strlen(alarm->mname)+1);
	memset(url,0,(strlen("/mnt/sdcard/alarm/") + strlen(alarm->mname)+1));
    sprintf(url, "/mnt/sdcard/alarm/%s",alarm->mname);	
	printf("\n--------------------alarm_Callback--------------------------------------\n");   
	printf("alarm callback id: %d, hour: %d, minute: %d, timestamp: %ld info: %s  url:%s\n",
...
//	mozart_musicplayer_play_music_info(mozart_musicplayer_handler, info);
	...
	do{		
		sleep(1);
		printf("\n.........this is enter do while...............\n");
		if(mozart_musicplayer_get_pos(mozart_musicplayer_handler) > (mozart_musicplayer_get_duration(mozart_musicplayer_handler)-3))
		{
			mozart_musicplayer_play_pause_pause(mozart_musicplayer_handler);
			alarm_end = 1;
		}
