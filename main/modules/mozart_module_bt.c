				...
				for(n=i+3 ; n<date_length+i+3 ; n++)
				{
					switch(date[i+n])
					{
						...
						case 0x22:														//Tower的电量数据(0x22)
						{
							...
							g_tower_power = info->Tower_power;
							info->Tower_if_power = date[i+n+2];
							g_tower_if_power = info->Tower_if_power;
						#endif
							info->Tower_power = date[i+n+1] & 0x7F;
							g_tower_power = info->Tower_power;
							info->Tower_if_power = (date[i+n+1] & 0x80) >> 7;
							g_tower_if_power = info->Tower_if_power;
							n=n+1;
							printf("info->Tower_if_power:%d,info->Tower_power:%d\n",info->Tower_if_power,info->Tower_power);
							break;
						}	
						...
						case 0xAD:											  		//Tower的WIFI连接状态信息(0xAD)
						{
							...
							memcpy(info->Tower_wlanIp,&date[i+n+3],6);
							printf("info->Tower_wlanIp:%s\n",info->Tower_wlanIp);
							#endif
							memcpy(info->Tower_WIFI_state,&date[i+n+2],length);
