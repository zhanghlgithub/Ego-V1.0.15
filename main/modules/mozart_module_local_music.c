...
void sleep_info_init()
{
	...
	if(info!=NULL)
    	strncpy(sleep_info.mname,info->music_name,strlen(info->music_name)+1);
		...
		break;  
	  	case(SLEEP):
		    ...
		    {	
				...
				if(mozart_musicplayer_get_status(mozart_musicplayer_handler) != PLAYER_STOPPED)
					mozart_musicplayer_stop_playback(mozart_musicplayer_handler);
				if(sleep_info.med_time > 30 || sleep_info.med_time == 300)
				{
...
int mozart_module_local_music_start(void)
{
	...
	//	snd_source = SND_SRC_LOCALPLAY;
		...
		{
			printf("【%s】 %s %d ..........................snd_source:%d\n",__FILE__, __func__, __LINE__,snd_source);
			return -1;
		}
...
//	scan = mozart_localplayer_is_scanning();
	...
	printf("从SD卡获取的音乐文件：%s\n",new_mozart_music_list);
	/*****************************************************************/
#if 0	
	if(!scan)
	{
