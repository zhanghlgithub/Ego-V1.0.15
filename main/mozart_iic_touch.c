...
#define ONE_KEY_SLEEP "/sys/class/gpio/gpio101/value"
...
static void *pthread_key_sleep(void *args)
{
	//初始化GPIO的设备号给应用层
	system("echo 101 > /sys/class/gpio/export"); 
	system("echo in > /sys/class/gpio/gpio101/direction"); 
...
void start_iic(void){
	int err;
	printf(">>>>>>>>>>>>>>>>>>pthread_key_sleep<<<<<<<<<<<<<<<<<<<\n");
	//err = pthread_create(&iic_pthread, NULL, pthread_iic, NULL);
	err = pthread_create(&iic_pthread, NULL, pthread_key_sleep, NULL);
	if (err != 0){
