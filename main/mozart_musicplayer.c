...
int mozart_musicplayer_start(musicplayer_handler_t *handler)
{
	...
	musicplayer_lock(&musicplayer_mutex);
//	printf("【%s】 %s %d ..........................current_player_handler.uuid:%s\n",__FILE__, __func__, __LINE__,current_player_handler.uuid);
...
//	int state_tower = 0;	//0：表示tower未连接，1：表示tower已连接
    while(1)
    {
#if 0
		if((1 == get_tower_state()) && (0 == state_tower))
		{
			printf("TOWER 连接成功\n");
			state_tower = 1;
			//蓝牙指示灯亮
		...
		if(0 ==  get_tower_state() && (1 == state_tower))
		{
			//蓝牙指示灯灭
			state_tower = 0;
			system("echo timer > /sys/class/leds/led-bt2/trigger");
